...
class ControlledLiteUnetModel(UNetModel):
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, **kwargs):
        ...
        h = x.type(self.dtype)
        input_layers_index_to_add = [1, 4, 7, 10]
        for i, module in enumerate(self.input_blocks):
            if control is not None and i in input_layers_index_to_add:
                h += control.pop(0)
...
class ControlLiteNet(nn.Module):
    def __init__(
        ...
        for level, (mult, stride,) in enumerate(zip(self.channel_mult, strides)):
            ...
            self.input_blocks_control.append(block)
        ...
        self.input_blocks_control.append(self.middle_block)
        self.zero_convs.append(self.make_zero_conv(ch))
        self._feature_size += ch
    ...
    def forward(self, x, hint, timesteps, context, **kwargs):
        ...
        for module, zero_conv in zip(self.input_blocks_control, self.zero_convs):
            h = module(h, emb, context)
